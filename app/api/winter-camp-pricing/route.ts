...
export async function GET(request: NextRequest) {
  try {
    ...
    const pricing = await prisma.winterCampPricing.findMany({
      orderBy: {
        createdAt: "desc",
      },
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const pricing = await prisma.winterCampPricing.create({
      data: {
    ...
    console.log("Winter camp pricing create error", error);
    return NextResponse.json(
      {
        message: "Failed to create winter camp pricing",
      },
      { status: 500 },
    );
  }
