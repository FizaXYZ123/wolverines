...
export async function PUT(
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: "id is required to update winter camp pricing",
        },
        { status: 400 },
      );
    }
    ...
    const existingPricing =
      await prisma.winterCampPricing.findUnique({
        where: {
          id,
        },
      });
    ...
    const finalMinAge =
      minAge !== undefined
        ? Number(minAge)
        : existingPricing.minAge;

    const finalMaxAge =
      maxAge !== undefined
        ? Number(maxAge)
        : existingPricing.maxAge;
    ...
    if (
      ...
      (Number(processingFeePercent) < 0 ||
        Number(processingFeePercent) > 100)
    ) {
      return NextResponse.json(
        {
          message:
    ...
    const updatedPricing =
      await prisma.winterCampPricing.update({
        where: {
          id,
        },
        data: {
    ...
    return NextResponse.json(
      {
        message: "Winter camp pricing updated successfully",
        data: updatedPricing,
      },
      { status: 200 },
    );
    ...
    console.error(
      "Error updating winter camp pricing:",
      error,
    );

    return NextResponse.json(
      {
        message: "Failed to update winter camp pricing",
      },
      { status: 500 },
    );
  }
...
export async function DELETE(request:NextRequest,context:RouteContext){
  try {
    ...
    if(!id){
      return NextResponse.json({
        message:"id is required to delete winter camp pricing"
      },{status:400})
    }

    const existingPricing = await prisma.winterCampPricing.findUnique({
      where:{
        id
      }
    })
   ...
   const deleted = await prisma.winterCampPricing.delete({
      where:{
        id
      }
    })
    ...
    return NextResponse.json({
      message:"Winter camp pricing deleted successfully",
      data:deleted
    },{status:200})
    ...
    console.error("Error deleting winter camp pricing:", error);

    return NextResponse.json(
      {
        message: "Failed to delete winter camp pricing",
      },
      { status: 500 },
    );
  }
